        ...
        with open(f'annotations/xmls/{f[:-4]}.xml') as file:
            ...
            if isinstance(info_xml['annotation']['object'], list):
                ...
                for object in info_xml['annotation']['object']:
                    box_dict = object['bndbox'] #TODO: Tomar en cuenta todas las cabezas
...
def build_callbacks():
    checkpointer = ModelCheckpoint(filepath='train.weights.h5', verbose=0, save_best_only=True, save_weights_only=True)
...
for i in range(k):
    start_split = (len(all_files)//k)*i
    end_split = (len(all_files)//k)*(i+1)

    #split into training and testing
    train_files = all_files[0:start_split] + all_files[end_split:]
    test_files  = all_files[start_split:end_split]
    ...
    plt.axis('off')
    plt.imshow(combined)
    plt.savefig(f'result_test_{i}.png')
